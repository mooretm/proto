...
# filtered noise
#axs2[0,2].plot(t_noise,filtered_noise[:-offset])
...
"""
# Subtract ffts to find amplitude differences at each frequency bin
amp_diff = fft_stimulus / fft_filtered_noise
# Apply the amplitude difference to the FFT of the filtered noise
fft_adj_filtered_noise = fft_filtered_noise * amp_diff
# Get time waveform of amplitude-corrected noise FFT
adj_filtered_noise = scipy.fft.irfft(fft_adj_filtered_noise,n=2048)
# Check outcome by taking another FFT of the adjusted time waveform
freqs_adj_filtered_noise, fft_adj_filtered_noise = ts.doFFT(adj_filtered_noise,fs)
print(f"RMS of stimulus FFT: {ts.rms(fft_stimulus)}")
print(f"RMS of adj filtered noise FFT: {ts.rms(fft_adj_filtered_noise)}")
print(f"RMS of stimulus: {ts.rms(stimulus)}")
print(f"RMS of adj filtered noise: {ts.rms(adj_filtered_noise)}")
"""
...
#FFT plots
plt.subplot(2,1,1)
plt.plot(freqs_stimulus,20*np.log10(fft_stimulus),color="blue",label="stimulus")
plt.plot(freqs_adj_filtered_noise,20*np.log10(fft_adj_filtered_noise),":",color="orange",label="Filtered Noise")
