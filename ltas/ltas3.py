...
#filt_fft = filt_fft - np.mean(filt_fft)

# Return filtered noise FFT to time domain
ifft_filt_fft = irfft(filt_fft)
#ifft_filt_fft = ifft_filt_fft - np.mean(ifft_filt_fft)
# Convert DTMF FFT to time domain for comparison
...
plt.plot(t,ifft_filt_fft)
plt.title('IFFT of shaped FFT')
# Original FFT of dtmf tones
plt.subplot(2,2,3)
plt.plot(freqs_dtmf,fft_dtmf)
plt.title('Original FFT of DTMF')
# IFFT of dtmf original FFT
plt.subplot(2,2,4)
plt.plot(t,ifft_dtmf)
plt.title('IFFT of DTMF FFT')
plt.show()
